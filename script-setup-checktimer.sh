...
echo -e "${Green}[-] Vui long chay: 'sudo systemctl restart check-time.service && sudo systemctl restart check-time.timer' neu ban nghi ngo script khong hoat dong!${RESET}"
